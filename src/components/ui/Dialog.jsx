...
// Wrapper que acepta "open" en lugar de "open" nativo de Radix
const Dialog = React.forwardRef(({ open, onOpenChange, children, ...props }, ref) => (
  <DialogPrimitive.Root open={open} onOpenChange={onOpenChange} {...props} ref={ref}>
    {children}
...
const DialogContent = React.forwardRef(({ className, children, hideCloseButton = false, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-gray-950 focus:ring-offset-2 disabled:pointer-events-none dark:ring-offset-gray-800 dark:focus:ring-gray-300">
          <X className="h-4 w-4" />
