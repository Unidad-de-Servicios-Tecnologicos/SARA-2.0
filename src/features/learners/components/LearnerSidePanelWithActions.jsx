...
const ActionButton = ({ icon, label, onClick, variant = "primary" }) => {
  const Icon = icon;
  const baseStyles = "w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg transition-colors";
  
  const variants = {
    primary: "text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-blue-200 dark:border-blue-900/30",
    secondary: "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-300 dark:border-gray-600",
  };
...
export default function LearnerSidePanel({
  ...
  return (
    <>
      ...
      {/* Panel lateral */}
      <div
        className={`fixed right-0 top-0 h-full w-full sm:w-96 bg-white dark:bg-gray-900 shadow-2xl z-40 overflow-y-auto transition-transform duration-300 ease-in-out ${
